...
Main.MainPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent : function(){
       //主界面采用border布局，分为north，south，west,center(border布局必须要有center)
    	this.northPart = new Ext.Panel( {
			region : 'north',
			border : false,
			html : '<div style="background:url('+BasePath+'static/images/main_north.gif) repeat-x; height:78px;">' +
					'<div align="right" style="height: 78"> <a href="'+BasePath+'logout"><h2>安全退出</h2></a></div>' +
					'</div>',
			height : 80
		});
    	...
    	this.southPart = new Ext.Panel( {
			region : 'south',
			html : '<div style="background:url('+BasePath+'static/images/main_south.gif) repeat-x; height:33px; ">'
	...
	var nodeLoader  = new Ext.tree.TreeLoader({
	     dataUrl:BasePath+'admin/menu/list' ,
	     baseParams: {} 
	}) ;
    //在树的节点加载时添加事件
	...
	this.simpleTree.on("click",this.node_click);
        ...
        this.centerTab =new Main.CenterTabPanel({
			...
			items : [{
				title : '我的首页',
				html : '<div style="width:1166px; height:428px; background:url('+BasePath+'static/images/3.jpg);"></div>'
			}]
        });
    ...
    node_click: function (node) {
        ...
        var pageUrl = node.attributes.menuUrl;
